    ...
    public class NetSimpleLoggerSOI : IServerObjectExtension, IRESTRequestHandler, IWebRequestHandler, IRequestHandler2, IRequestHandler
    {
            ...
            string operationInput, string outputFormat, string requestProperties, out string responseProperties)
        {
            try
            {
                ...
                IServerUserInfo userinfo = ServerUtilities.GetServerEnvironment().UserInfo;

                String user = userinfo.Name;
                var roles = ServerUtilities.GetRoleInformation(userinfo);
                string resultRoles = String.Join(", ", roles);
                _serverLog.LogMessage(ServerLogger.msgType.debug, _soiName + ".HandleRESTRequest()", 200, resultRoles);
